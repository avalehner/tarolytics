...
usersRouter.patch("/:userId", async (req: Request, res: Response) => {
  try {
    ...
    const dbResponse = await pool.query(
      `
      UPDATE users 
      SET birthday = $1, birth_time = $2, birth_timezone = $3, birth_location = $4
      WHERE id =  $5`,
      [birthday, birth_time, birth_timezone, birth_location, userId],
    );

    const updatedUser = await dbResponse.rows[0];
