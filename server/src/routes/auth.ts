...
authRouter.get("/google/callback", async (req: Request, res: Response) => {
  try {
    ...
    if (!googleResponse.ok)
      throw new Error(`Auth server error: ${googleResponse.status}`);
    const googleObj = await googleResponse.json(); //contains access token

    const userInfoResponse = await fetch(
      `https://openidconnect.googleapis.com/v1/userinfo`,
      {
        //end point will return user name, picture, and email
        method: "GET",
    ...
    if (!userInfoResponse.ok)
      throw new Error(`Auth server error: ${userInfoResponse.status}`);
...
authRouter.get("/me", async (req: Request, res: Response) => {
  ...
  if (!token)
    return res
      .status(401) // if no token return error
  ...
  try {
    ...
    res.status(200).json(userData); //returns user data
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
