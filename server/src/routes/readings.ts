...
readingsRouter.post('/', async (req: Request, res: Response) => {
  try {
    const { reading_date, reading_topic, spread_type, notes } = req.body //destructure request object 
    const dbResponse = await pool.query(`
      INSERT INTO readings (reading_date, reading_topic, spread_type, notes)
      VALUES ($1, $2, $3, $4)
      RETURNING *;`, 
      [reading_date, reading_topic, spread_type, notes]
...
readingsRouter.patch('/:readingId', async (req: Request, res: Response) => { //':' marks a route parameter 
  try {
    ...
    const { reading_date, reading_topic, spread_type, notes } = req.body 

    const dbResponse = await pool.query(`
      UPDATE readings 
      SET reading_date = $1, reading_topic = $2, spread_type = $3, notes = $4
      WHERE id = $5
      RETURNING *;`, 
      [reading_date, reading_topic, spread_type, notes, readingId]
    )
